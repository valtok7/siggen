...
def rate_conv(iq, up_rate, down_rate, filter_length=51, log=False):
    """
    Rate Conversion
    resampleはsyncフィルタを用いるのに対して、本関数はInterpolate+Decimateによりレート変換する。
    up_rate/down_rateのレート変換が行われる。
    ...
    start_position : int
        ...
        出力データ（converted_iq）にはフィルタの過渡応答が含まれる。
        start_positionは過渡応答の除いた開始位置が入る。
    """
    # Prepare a filter
